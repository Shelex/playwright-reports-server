...
export async function readResults() {
	...
	>(
		...
		async (file) => {
			...
			const sizeBytes = await getFolderSize.loose(
				filePath.replace(".json", ".zip"),
			);
...
async function readOrParseReportMetadata(
	id: string,
	projectName: string,
	...
	if (metadataError)
		console.error(
			`failed to read metadata for ${id}: ${metadataError.message}`,
		);
	...
	try {
		const parsed = await parseReportMetadata(
			id,
...
export async function readReport(
	...
	const { result: metadataContent, error: metadataError } = await withError(
		readFile(path.join(reportPath, REPORT_METADATA_FILE), "utf-8"),
	);

	if (metadataError) {
		console.error(
			`[fs] failed to read metadata for ${reportID}: ${metadataError.message}`,
		);
...
export async function readReports(): Promise<ReadReportsOutput> {
	...
	const reportEntries = entries.filter(
		(entry) =>
			...
			!(entry as any).path.endsWith("trace"),
	);

	const stats = await processBatch<
		Dirent,
		Stats & { filePath: string; createdAt: Date }
	>({}, reportEntries, 20, async (file) => {
...
export async function saveResultDetails(
	...
	if (writeJsonError) {
		throw new Error(
			`failed to save result ${resultID} json file: ${writeJsonError.message}`,
		);
	}
...
async function saveReportMetadata(reportPath: string, info: ReportMetadata) {
	return fs.writeFile(
		path.join(reportPath, REPORT_METADATA_FILE),
...
async function saveConfigFile(config: Partial<SiteWhiteLabelConfig>) {
	...
	const isConfigFailed =
		!!configError && configError?.message !== noConfigErr && !existingConfig;

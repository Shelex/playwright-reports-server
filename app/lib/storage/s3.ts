...
import type { SiteWhiteLabelConfig } from "@/app/types";
...
export class S3 implements Storage {
	...
	async clear(...path: string[]) {
		...
		await processBatch<string, void>(
			this,
			path,
			this.batchSize,
			async (object) => {
		...
		do {
			...
			for (const obj of response.Contents ?? []) {
				...
				if (
					...
					!obj.Key.includes("/trace/index.html")
				) {
					indexCount += 1;
				}
	...
	async readReport(
		...
		if (downloadError) {
			console.error(
				`[s3] failed to download ${objectKey}: ${downloadError.message}`,
			);

			throw new Error(
				`failed to download ${objectKey}: ${downloadError.message}`,
			);
		}
		...
		try {
			...
			console.error(
				`[s3] failed to read or parse metadata file: ${(e as Error).message}`,
			);
	...
	async readReports(): Promise<ReadReportsOutput> {
		...
		const withMetadata = await this.getReportsMetadata(
			reports as ReportHistory[],
		);
	...
	async readOrParseReportMetadata(
		id: string,
		projectName: string,
	): Promise<ReportHistory> {
		const { result: metadataContent, error: metadataError } = await withError(
			this.readFile(
				path.join(REPORTS_BUCKET, projectName, id, REPORT_METADATA_FILE),
				"utf-8",
			),
		);
		...
		if (metadataError)
			console.error(
				`[s3] failed to read metadata for ${id}: ${metadataError.message}`,
			);

		const metadata =
			metadataContent && !metadataError
		...
		try {
			const { result: htmlContent, error: htmlError } = await withError(
				this.readFile(
					path.join(REPORTS_BUCKET, projectName, id, "index.html"),
					"utf-8",
				),
			);
			...
			if (htmlError)
				console.error(
					`[s3] failed to read index.html for ${id}: ${htmlError.message}`,
				);
			...
			console.log(
				`metadata object created for ${id}: ${JSON.stringify(created)}`,
			);
			...
			console.error(
				`failed to create metadata for ${id}: ${(e as Error).message}`,
			);
		}
	...
	async saveResult(filename: string, stream: PassThrough) {
		...
		console.log(
			`[s3] starting multipart upload for ${filename} with chunk size ${chunkSizeMB}MB`,
		);
		...
		if (!uploadID) {
			throw new Error(
				"[s3] failed to initiate multipart upload: no UploadId received",
			);
		}
		...
		try {
			for await (const chunk of stream) {
				...
				while (currentSize >= chunkSize) {
					...
					if (!uploadPartResult.ETag) {
						throw new Error(
							`[s3] failed to upload part ${partNumber}: no ETag received`,
						);
					}
			...
			if (currentSize > 0) {
				...
				if (!uploadPartResult.ETag) {
					throw new Error(
						`[s3] failed to upload final part ${partNumber}: no ETag received`,
					);
				}
			...
			console.log(
				`[s3] multipart upload completed successfully for ${filename}`,
			);
		} catch (error) {
			console.error(
				`[s3] multipart upload failed, aborting: ${(error as Error).message}`,
			);
		...
		await processBatch(this, files, this.batchSize, async (file) => {
			...
			const { error } = await withError(
				this.uploadFileWithRetry(
					s3Path,
					path.join((file as any).path, file.name),
				),
			);
	...
	private async uploadFileWithRetry(
		...
		if (attempt > 3) {
			throw new Error(
				`[s3] failed to upload file after ${attempt} attempts: ${filePath}`,
			);
		}
	...
	private async clearTempFolders(id?: string) {
		...
		await withError(
			fs.rm(path.join(TMP_FOLDER, id ?? ""), { recursive: true, force: true }),
		);
	...
	async generateReport(
		...
		console.log(
			`[s3] generate report from results: ${JSON.stringify(resultsIds)}`,
		);
		...
		if (mkdirReportsError) {
			console.error(
				`[s3] failed to create reports folder: ${mkdirReportsError.message}`,
			);
		}
		...
		const { error: mkdirTempError } = await withError(
			fs.mkdir(tempFolder, { recursive: true }),
		);

		if (mkdirTempError) {
			console.error(
				`[s3] failed to create temporary folder: ${mkdirTempError.message}`,
			);
		}
		...
		for (const resultId of resultsIds) {
			...
			if (headError) {
				console.error(
					`[s3] result ${resultId} not found, skipping: ${headError.message}`,
				);
				throw new Error(`failed to check ${objectKey}: ${headError.message}`);
			}
		...
		if (parseReportMetadataError)
			console.error(parseReportMetadataError.message);
		...
		if (uploadError) {
			...
			if (error)
				console.error(`[s3] failed to save report metadata: ${error.message}`);
		}
	...
	private async saveReportMetadata(
		...
		console.log(
			`[s3] report uploaded: ${reportId}, uploading metadata to ${reportPath}`,
		);
		...
		if (metadataError)
			console.error(
				`[s3] failed to upload report metadata: ${metadataError.message}`,
			);
	}
	...
	private async parseReportMetadata(
		...
		console.log(
			`[s3] creating report metadata for ${reportId} and ${reportPath}`,
		);
		const html =
			htmlContent ??
			(await fs.readFile(path.join(reportPath, "index.html"), "utf-8"));
		...
		try {
			...
			for (const image of [
				...
				if (accessError) {
					...
					console.log(
						`[s3] downloading config image: ${remotePath} to ${localPath}`,
					);
	...
	async saveConfigFile(config: Partial<SiteWhiteLabelConfig>) {
		...
		const { result: existingConfig, error: readExistingConfigError } =
			await this.readConfigFile();

		if (readExistingConfigError) {
			console.error(
				`[s3] failed to read existing config file: ${readExistingConfigError.message}`,
			);
		}

		const { error: clearExistingConfigError } = await withError(
			this.clear(APP_CONFIG_S3),
		);
		...
		const shouldBeUploaded = async (key: keyof SiteWhiteLabelConfig) => {
			...
			const imagePath =
				key === "logoPath" ? uploadConfig.logoPath : uploadConfig.faviconPath;
		...
		if (error)
			console.error(`[s3] failed to write config file: ${error.message}`);
	...
	private async uploadConfigImage(imagePath: string): Promise<Error | null> {
		...
		const { error } = await withError(
			this.uploadFileWithRetry(remotePath, localPath),
		);

	...
	const handleSave = async (section: "server" | "jira" | "cron") => {
		...
		try {
			...
			if (section === "server") {
				...
				if (
					tempConfig.faviconPath &&
					tempConfig.faviconPath !== config.faviconPath
				) {
				...
				const cleanHeaderLinks = Object.fromEntries(
					Object.entries(tempConfig.headerLinks || {}).filter(

...
import type { ReportFile, ReportTest } from "@/types/parser";
import type { ReportHistory } from "@/types/storage";
...
function buildTestTree(rootName: string, tests: ReportTest[]): SuiteNode {
	...
	tests.forEach((test) => {
		...
		path.reduce((currentNode: SuiteNode, suiteName: string, index: number) => {
			const existingSuite = currentNode.children.find(
				(child) => child.name === suiteName,
			);

...
const getMetadataItems = (item: ReportHistory) => {
	...
	if (itemWithMetadata.workingDir) {
		const dirName =
			itemWithMetadata.workingDir.split("/").pop() ||
			itemWithMetadata.workingDir;
	...
	Object.entries(itemWithMetadata).forEach(([key, value]) => {
		if (
			...
			!["environment", "workingDir", "branch"].includes(key)
		) {
...
export default function ReportsTable({
	...
	const {
		...
		withQueryParams(reportListEndpoint, getQueryParams()),
		{

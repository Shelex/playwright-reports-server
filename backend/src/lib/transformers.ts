...
const isTestMatchingFilters = (
	...
	const byOutcome =
		!filters.outcomes || filters.outcomes.includes(test.outcome);
	const byTitle =
		!filters.name ||
		test.title.toLowerCase().includes(filters.name.toLowerCase());
...
export const filterReportHistory = (
	...
	const filteredFiles = filtered.files.reduce((files, file) => {
		...
		const filteredTests = file.tests.filter((test) =>
			isTestMatchingFilters(test, filters),
		);

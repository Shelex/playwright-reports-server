...
export class JiraService {
	...
	public async createIssue(
		...
		const issueTypeObj = availableIssueTypes.find(
			(it: any) => it.name === issueType,
		);
	...
	public async addAttachment(
		...
		try {
			...
			console.log(
				`Successfully attached ${attachment.name} to issue ${issueKey}`,
			);

			return result;
		} catch (error) {
			console.error(
				`Error attaching file ${attachment.name} to issue ${issueKey}:`,
				error,
			);
			throw error;
		}

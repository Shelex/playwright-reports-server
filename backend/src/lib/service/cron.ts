...
export class CronService {
	...
	public async init() {
		...
		const reportExpireDays =
			cfg.cron?.reportExpireDays || env.REPORT_EXPIRE_DAYS;
		const resultExpireDays =
			cfg.cron?.resultExpireDays || env.RESULT_EXPIRE_DAYS;
	...
	private clearReportsTask() {
		...
		return this.createJob(scheduleExpression, async () => {
			...
			if (error)
				console.error(`[cron-job] error deleting outdated results: ${error}`);
		});
	}
	...
	private clearResultsTask() {
		...
		return this.createJob(scheduleExpression, async () => {
			...
			const outdated = resultsOutput.results
				...
				.filter((result) =>
					expireDays ? this.isExpired(result.createdDate, expireDays) : false,
				);

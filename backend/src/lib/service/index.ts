...
class Service {
	...
	public async getReport(id: string, path?: string) {
		...
		if (!report && path) {
			console.warn(
				`[service] getReport ${id} - not found in db, fetching from storage`,
			);
			const { result: reportFromStorage, error } = await withError(
				storage.readReport(id, path),
			);

			if (error) {
				console.error(
					`[service] getReport ${id} - error fetching from storage: ${error.message}`,
				);
				throw error;
			}
	...
	private async findLatestPlaywrightVersionFromResults(resultIds: string[]) {
		for (const resultId of resultIds) {
			const { result: results, error } = await withError(
				this.getResults({ search: resultId }),
			);
	...
	private async findLatestPlaywrightVersion(resultIds: string[]) {
		const versionFromResults =
			await this.findLatestPlaywrightVersionFromResults(resultIds);
	...
	public async generateReport(
		...
		const { reportId, reportPath } = await storage.generateReport(
			resultsIds,
			metadataWithVersion,
		);

		console.log(
			`[service] reading report ${reportId} from path: ${reportPath}`,
		);
		const { result: report, error } = await withError(
			storage.readReport(reportId, reportPath),
		);
		...
		const projectPath = metadata?.project
			? `${encodeURI(metadata.project)}/`
			: "";
	...
	public async getResults(
		input?: ReadResultsInput,
	): Promise<ReadResultsOutput> {
	...
	public async deleteResults(resultIDs: string[]): Promise<void> {
		...
		if (error) {
			console.error(
				`[service] deleteResults - storage deletion failed:`,
				error,
			);
			throw error;
		}
	...
	public async saveResult(
		...
		console.log(
			`[service] using direct upload via presigned URL`,
			presignedUrl,
		);
	...
	public async saveResultDetails(
		resultID: string,
		resultDetails: ResultDetails,
		size: number,
	) {
		const result = await storage.saveResultDetails(
			resultID,
			resultDetails,
			size,
		);

...
export class ResultDatabase {
    ...
    `);
		...
		this.getAllStmt = this.db.prepare(
			"SELECT * FROM results ORDER BY createdAt DESC",
		);
	...
	public async init() {
		...
		const { result: resultsResponse, error } = await withError(
			storage.readResults(),
		);
		...
		console.log(
			`[result db] caching ${resultsResponse.results.length} results`,
		);
	...
	public query(input?: ReadResultsInput): ReadResultsOutput {
		...
		if (input?.pagination) {
			query += " LIMIT ? OFFSET ?";
			params.push(
				input.pagination.limit.toString(),
				input.pagination.offset.toString(),
			);
		}

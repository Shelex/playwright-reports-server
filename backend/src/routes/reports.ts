					...
					Object.entries(validatedBody)
						.filter(
							([key]) =>
								...
								typeof validatedBody[key as keyof typeof validatedBody] ===
									"string",
						)
	...
	fastify.delete("/api/report/delete", async (request, reply) => {
		try {
			...
			if (!body.reportsIds || !Array.isArray(body.reportsIds)) {
				return reply
					.status(400)
					.send({ error: "reportsIds array is required" });
			}

			if (body.reportsIds.length === 0) {
				return reply
					.status(400)
					.send({ error: "At least one report ID must be provided" });
			}
			...
			const { error } = await withError(
				service.deleteReports(validatedBody.reportsIds),
			);

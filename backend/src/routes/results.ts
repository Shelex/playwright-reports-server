...
export async function registerResultRoutes(fastify: FastifyInstance) {
	...
	fastify.get("/api/result/projects", async (_, reply) => {
		const { result: projects, error } = await withError(
			service.getResultsProjects(),
		);
		...
		async (request, reply) => {
			try {
				...
				if (!body.resultsIds || !Array.isArray(body.resultsIds)) {
					return reply
						.status(400)
						.send({ error: "resultsIds array is required" });
				}

				if (body.resultsIds.length === 0) {
					return reply
						.status(400)
						.send({ error: "At least one result ID must be provided" });
				}
				...
				const { error } = await withError(
					service.deleteResults(validatedBody.resultsIds),
				);
	...
	fastify.put("/api/result/upload", async (request, reply) => {
		...
		const presignedUrl = contentLength
		...
		try {
			...
			if (!data) {
				return reply
					.status(400)
					.send({ error: "upload result failed: No file received" });
			}
			...
			const { result: uploadResult, error: uploadResultDetailsError } =
				await withError(
			...
			if (
				...
				const testRunResults = results?.results.filter(
					(result) =>
						...
						(resultDetails.project
							? result.project === resultDetails.project
							: true),
				);
				...
				if (
					testRunResults?.length === Number.parseInt(resultDetails.shardTotal)
				) {
					const ids = testRunResults.map((result) => result.resultID);

					console.log(
						"triggerReportGeneration for",
						resultDetails.testRun,
						ids,
					);
					const { result, error } = await withError(
					...
					if (error) {
						return reply
							.status(500)
							.send({ error: `failed to generate report: ${error.message}` });
					}
			...
			const { error: deleteError } = await withError(
				service.deleteResults([resultID]),
			);
			if (deleteError) {
				console.error(
					`[upload] cleanup failed for result ${resultID}:`,
					deleteError,
				);

...
export async function registerJiraRoutes(fastify: FastifyInstance) {
		...
		try {
			...
			if (!isConfigured) {
				return reply.send({
					configured: false,
					message:
						"Jira is not configured. Please configure Jira settings in the admin panel.",
					config: jiraConfig || {},
				});
			}
	...
	fastify.post("/api/jira/create-ticket", async (request, reply) => {
		...
		const { result: data, error: parseError } = await withError(
		...
		try {
			...
			fastify.log.error(
				{ error },
				`Failed to get report ${ticketData.reportId}`,
			);
		}
		...
		try {
			...
			if (
				...
				error.message.includes("Jira configuration is incomplete")
			) {
				return reply.status(500).send({
					error:

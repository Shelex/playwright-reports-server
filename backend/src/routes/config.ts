			...
			try {
				...
				if (logoFile) {
					const { error: logoError } = await withError(
						writeFile(
							join(DATA_FOLDER, logoFile.filename!),
							Buffer.from(await logoFile.toBuffer()),
						),
					);
					...
					if (logoError) {
						return reply
							.status(500)
							.send({ error: `failed to save logo: ${logoError?.message}` });
					}
				...
				if (formData.jiraBaseUrl !== undefined)
					config.jira.baseUrl = formData.jiraBaseUrl;
				if (formData.jiraEmail !== undefined)
					config.jira.email = formData.jiraEmail;
				if (formData.jiraApiToken !== undefined)
					config.jira.apiToken = formData.jiraApiToken;
				if (formData.jiraProjectKey !== undefined)
					config.jira.projectKey = formData.jiraProjectKey;
				...
				if (formData.resultExpireDays !== undefined) {
					config.cron.resultExpireDays = Number.parseInt(
				...
				if (formData.reportExpireDays !== undefined) {
					config.cron.reportExpireDays = Number.parseInt(
				...
				const { error: saveConfigError } = await withError(
